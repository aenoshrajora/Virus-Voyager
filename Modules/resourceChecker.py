...
class ResourceScanner:
    ...
    def method_1_replace_split(self, r1, r2, sp1, executable_buffer):
        ...
        self.save_data_into_file("virusvoyager_carved_deobfuscated.exe", sanitized_data)
    ...
    def method_7_base64_and_reverse(self, executable_buffer):
        ...
        # Save data
        with open("virusvoyager_carved_deobfuscated.exe", "wb") as ff:
            ff.write(final_buffer)
        print(f"{infoS} Data saved into: [bold green]virusvoyager_carved_deobfuscated.exe[white]\n")
    ...
    def windows_resource_scanner_split_data_carver_method(self):
        ...
        if founder_switch != 0:
            ...
            self.save_data_into_file("virusvoyager_carved_deobfuscated_split.exe", output_buffer)
        else:
    ...
    def bitmap_carver_1(self, image_handler):
        if os.path.exists("carved.bmp"):
            ...
            if b"4d5a90" in binascii.hexlify(b_array):
                ...
                with open("virusvoyager_hidden_pe.exe", "wb") as ff:
                    ff.write(b_array)
                print(f"{infoS} Data saved into: [bold green]virusvoyager_hidden_pe.exe[white]\n")
    ...
    def windows_resource_scanner_locate_encrypted(self):
        ...
        for artifact in encrypted_sigs:
            ...
            if offsets != []:
                ...
                with open(f"virusvoyager_carved_decrypted-{hex(offsets[0])}.exe", "wb") as ff:
                    ff.write(decr)
                print(f"{infoS} Data saved into: [bold green]virusvoyager_carved_decrypted-{hex(offsets[0])}.exe[white]\n")
        if founz == 0:
