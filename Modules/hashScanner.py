...
def DatabaseUpdate():
    if os.path.exists(install_dir):
        ...
        print(f"[bold magenta]>>>[white] If you don\'t want to execute [bold green]{setup_scr}[white] then try this: [bold green]python virusvoyager.py --file your_sample --hashscan[white]")
...
def MultipleScan():
    ...
    if os.path.isdir(targetFile) == True:
        ...
        print("[bold red]>>>[bold white] virusvoyager gathering all files under that directory recursively. [bold blink]Please wait...")
        scanfiles = Table()
        ...
        upper_panel = Panel(upper_grid, border_style="bold blue", title="V1rusV0y1g3r Hashscan")
