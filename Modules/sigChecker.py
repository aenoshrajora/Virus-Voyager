...
class SignatureChecker:
    ...
    def file_carver_for_windows_executables(self, offset_array):
        ...
        for off in self.offset_array:
            ...
            # Creating dump files
            try:
                dumpfile = open(f"virusvoyager_carved-{off}.bin", "wb")
                buffer_to_write = pfile.trim()
                dumpfile.write(buffer_to_write)
                dumpfile.close()
                pfile.close()
                print(f"[bold magenta]>>>[white] Data saved into: [bold green]virusvoyager_carved-{off}.bin")
            except:
                continue
        ...
        with open(self.target_file, "rb") as target:
            ...
            if detected_executables != []:
                ...
                for binary in detected_executables:
                    ...
                    # Creating dump files
                    try:
                        dumpfile = open(f"virusvoyager_carved-{binary[0]}.bin", "wb")
                        buffer_to_write = pfile.trim()
                        ...
                        print(f"[bold magenta]>>>[white] Data saving into: [bold green]virusvoyager_carved-{binary[0]}.bin[white] | Size: [bold green]{len(buffer_to_write)}[white]")
                        dumpfile.write(buffer_to_write)
                        dumpfile.close()
                        pfile.close()
                    except:
    ...
    def file_carver_for_elf_executables(self, offset_array):
        for ofs in offset_array:
            ...
            if binary is not None:
                with open(f"virusvoyager_carved_ELF-{hex(ofs)}.bin", "wb") as ff:
                    ff.write(self.getbins_buffer[ofs:binary.eof_offset+ofs])
                print(f"[bold magenta]>>>[white] Data saved into: [bold green]virusvoyager_carved_ELF-{hex(ofs)}.bin")
