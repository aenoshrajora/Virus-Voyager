...
class DocumentAnalyzer:
    ...
    def OneNoteAnalysis(self):
        ...
        for key in efs.keys():
            with open(f"virusvoyager_carved-{key}{efs[key]['extension']}", "wb") as binfile:
                binfile.write(efs[key]["content"])
            binfile.close()
            print(f"[bold magenta]>>>[white] Embedded file saved as: [bold green]virusvoyager_carved-{key}{efs[key]['extension']}[white]")
                    ...
                    try:
                            ...
                            for categ in self.file_sigs:
                                for pattern in self.file_sigs[categ]["patterns"]:
                                    ...
                                    if regex != []:
                                        ...
                                        print(f"{infoS} Attempting to extraction...")
                                        self.output_writer(out_file=f"virusvoyager_carved-{categ}-{obj}.bin", mode="wb", buffer=object_data)
                        ...
                        if "EmbeddedFile" in str(doc.getobj(obj)) and "PDFStream" in str(doc.getobj(obj)):
                            ...
                            try:
                                ...
                                self.output_writer(out_file=f"virusvoyager_embedded_decompressed_file-{obj}.bin", mode="wb", buffer=decompressed)
                            except:
                                self.output_writer(out_file=f"virusvoyager_embedded_file-{obj}.bin", mode="wb", buffer=doc.getobj(obj).get_rawdata())
                    except:
                        continue
            else:
                pass
        ...
        if self.mal_code["unescape"]["count"] != 0:
            ...
            if un_dat != []:
                for escape in un_dat:
                    ...
                    self.output_writer(out_file=f"virusvoyager_decoded_unescape-{len(deobf)}.bin", mode="w", buffer=deobf)

                    # After extracting the data also we need to scan it!
                    print(f"\n{infoS} Performing analysis against [bold yellow]virusvoyager_decoded_unescape-{len(deobf)}.bin[white]")
                    if "html" in deobf:
                        ...
                        if len(decoded.decode()) <= 6 and (")" in decoded.decode() or "(" in decoded.decode() or "[" in decoded.decode() or "]" in decoded.decode() or "+" in decoded.decode() or "-" in decoded.decode() or "<" in decoded.decode() or ">" in decoded.decode() or "*" in decoded.decode() or "!" in decoded.decode()):
                            ...
                            if key_count != 0:
                                if len(decoded.decode()) >= 150:
                                    ...
                                    self.output_writer(out_file=f"virusvoyager_decoded_javascript-{len(decoded.decode())}.js", mode="w", buffer=decoded.decode())
                                else:
        ...
        if javscr != []:
            ...
            for jv in javscr:
                ...
                if mal_ind != 0 and len(jav_buf) > 0:
                    self.output_writer(out_file=f"virusvoyager_carved_javascript-{len(jav_buf)}.js", mode="w", buffer=jav_buf)
        else:
        ...
        for exp_pattern in self.rtf_exploit_extract_dict:
            for pattern in self.rtf_exploit_extract_dict[exp_pattern]["pattern"]:
                ...
                if chk_ex != []:
                    ...
                    self.pat_ct += 1
                    self.output_writer(out_file=f"virusvoyager_extracted_exploit-{len(buffer)}.bin", mode="wb", buffer=binascii.unhexlify(buffer))
    ...
    def rtf_check_exploit_main(self, buffer):
        ...
        if get_brackets != []:
            ...
            self.output_writer(out_file=f"virusvoyager_extracted_exploit-{len(finalbuffer)}.bin", mode="wb", buffer=finalbuffer)
            ...
            if self.mal_rtf_code["unescape"]["count"] != 0:
                ...
                if unesc != []:
                    ...
                    for un in unesc:
                        ...
                        self.output_writer(out_file=f"virusvoyager_deobfuscated_unescape-{len(deobf)}.bin", mode="w", buffer=deobf)
        else:
