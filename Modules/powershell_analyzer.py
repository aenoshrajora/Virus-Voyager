...
# Gathering virusvoyager path variable
...
class PowerShellAnalyzer:
    ...
    def xor_decrypt_and_save(self, payload_type, payload, xor_key):
        if payload_type == "base64":
            ...
            self.save_data_into_file(output_file="virusvoyager_decoded_b64_payload.bin", data=byte_arr)
            ...
            for byt in range(len(byte_arr)):
                byte_arr[byt] = byte_arr[byt] ^ int(xor_key)
            self.save_data_into_file(output_file="virusvoyager_decoded_ascii_numbers_payload.bin", data=byte_arr)
        elif payload_type == "hex":
            byte_arr = bytearray(binascii.unhexlify(payload))
            for byt in range(len(byte_arr)):
                byte_arr[byt] = byte_arr[byt] ^ int(xor_key)
            self.save_data_into_file(output_file="virusvoyager_decoded_hex_values_payload.bin", data=byte_arr)
        else:
            pass
    ...
    def detect_and_carve_b64_non_xored(self):
        ...
        if b64matches != []:
            ...
            if deflatestream != []:
                ...
                self.save_data_into_file(output_file="virusvoyager_decoded_b64_payload.bin", data=output.encode())
            elif gzipstream != []:
                print(f"{infoS} Gzip data found! Attempting to decompress...")
                decompressed_data = gzip.decompress(b64_data)
                self.save_data_into_file(output_file="virusvoyager_decoded_b64_payload.bin", data=decompressed_data)
            else:
                print(f"{infoS} There is no compression. Extracting payload anyway...")
                self.save_data_into_file(output_file="virusvoyager_decoded_b64_payload.bin", data=b64_data)
        else:
            print(f"{errorS} There is no pattern about BASE64 encoded payloads!\n")
            ...
            for enc in b64_match:
                try:
                    ...
                    if len(decbf.decode()) > 10:
                        with open(f"virusvoyager_decoded_b64_{len(decbf.decode())}.bin", "w") as ff:
                            ff.write(decbf.decode())
                        print(f"{infoS} Decoded payload saved into: [bold green]virusvoyager_decoded_b64_{len(decbf.decode())}.bin[white]")
                except:
                    continue
        else:
            print(f"{errorS} There is no pattern about BASE64 encoded payloads!\n")
        ...
        if pe_match != []:
            ...
            for pat in self.all_strings:
                if "4D5A90" in pat and "=" in pat:
                    ...
                    self.save_data_into_file(output_file=f"virusvoyager_extracted_pe_{counter}.exe", data=binascii.unhexlify(sanitized))
                    counter += 1
        else:
