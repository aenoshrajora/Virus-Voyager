...
# Make VirusVoyager work on Windows, Linux, OSX
...
# Is VirusVoyager installed??
if os.name != "nt":
    if os.path.exists("/usr/bin/virusvoyager") == True and os.path.exists(f"/etc/virtusvoyager.conf") == True:
        # Parsing new path and write into handler
        sc0peConf = configparser.ConfigParser()
        sc0peConf.read(f"/etc/virusvoyager.conf")
        sc0pe_path = str(sc0peConf["virusvoyager_PATH"]["sc0pe"])
...
ARG_NAMES_TO_KWARG_OPTS = {
    ...
    "console": {"help": "Use virusvoyager on interactive shell.", "action": "store_true"},
    ...
    "install": {"help": "Install or Uninstall virusvoyager.", "action": "store_true"},
...
def virusvoyager():
    ...
    # Install virusvoyager on your system!!
    if args.install:
